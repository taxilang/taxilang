...
repositories {
   ...
   // Only in CI, add the Maven output as a local repo
   val mavenLocalRepo = System.getProperty("MAVEN_LOCAL_REPO")
   if (mavenLocalRepo != null && file(mavenLocalRepo).exists()) {
      maven {
         url = uri(file(mavenLocalRepo))
      }
      println("Using Maven local repo at: $mavenLocalRepo")
   } else {
      println("No custom Maven local repo detected")
   }
