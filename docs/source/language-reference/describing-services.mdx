...
## Query operations
RESTful style API's are great for discovering resources that have fairly predictable resource paths - such as `/Author/jkrowling/Books`, returns all the books that `jkrowling` has authored.
 ...
 * An operation named `personQuery`, which accepts a `VyneQlQuery`
...
From taxi's perspective, the name of the grammar being used is entirely arbitary.  The query server that is
invoking endpoints must understand how to construct queries in the described language.

The Vyne query server understands how to construct <Link to="../querying-with-taxiql">taxiQl</Link> queries, with support for additional grammars (such as `SQL`, and `GraphQL`) planned.
