...
<Hint>
   ...
   **Important:** Throughout taxi, we favour using Types, rather than field names, to describe attributes of data - it's
...
## Anonymous projected types
When writing a query, it's very common to define a custom result type, rather than using an existing type defined elsewhere in the schema.
These custom types are called Anonymous types, and allow for structuring the result exactly as you need it, along with mixing and matching content from multiple different sources.  (It's up to the query engine to determine how to resolve these relationships.)


An example of a simple anonymous type is as follows:
...
## Linking data and `@Id`
<Hint>This behaviour is down to server engines. What's documented is the reference behaviour as implemented by Vyne.
