...
The spread operator (`...`) is shorthand for "And everything else from that type".

For example, given a Person model:
...
using the spread operator as follows would return all fields from the Person model along with the reviewRating
...
The spread operator can also be used to exclude certain fields from a projection:
<Hint>
   Note that exclusion works using field names, not types.
</Hint>
