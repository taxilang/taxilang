...
When a view definition contains multiple ‘find’ statements, each ‘find’ statement body must be compatible with each other so that Vyne can perform ‘union’ operation to combine the result of each ‘find’ statement. If two ‘field’ statements are compatible, then:
