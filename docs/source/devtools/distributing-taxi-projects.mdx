...
For more details on how to define dependencies, and configure repositories to download them from, see <Link to="/command-line-tools/taxi-conf-file">here</Link>
...
Credentials can be stored elsewhere, following the rules for `taxi.conf` merging, as discussed <Link to="/command-line-tools/taxi-conf-file/#credentials">here</Link>.
