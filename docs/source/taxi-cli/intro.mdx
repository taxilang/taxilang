...
Orbital is automatically configured with the following components:

 * The [Orbital UI](https://orbitalhq.com), which allows you to browse and search the taxonomy as you author
 * A local [schema server](https://orbitalhq.com/docs/deploying/schema-server), which will live-sync changes as you make them in your local filesystem.
