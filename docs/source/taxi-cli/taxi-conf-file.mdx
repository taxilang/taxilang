...
<Discourage>
   Transitive dependencies (ie., depending on projects that depend on other
   projects) are not yet supported, but are on the roadmap. If this feature is
   important to you, get in touch on our slack channel.
</Discourage>
...
<Discourage>
   When making changes in VSCode, remember that you must save the `taxi.conf`
