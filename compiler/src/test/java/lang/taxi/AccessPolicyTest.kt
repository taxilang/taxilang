...
//        expect(policy.rules).to.have.size(3)
//        val ruleSet = policy.rules[0]
//        expect(ruleSet.statements).to.have.size(4)
//        expect(ruleSet.operationScope.operationType).to.equal("read")
//        expect(ruleSet.operationScope.policyOperationScope).to.equal(PolicyOperationScope.EXTERNAL)
//        expect(policy.rules[0].statements).to.have.size(4)
//
...
//        val policy = doc.policy("test.TradeDeskPolicy")
//        val ruleSet = policy.rules.first { it.operationScope.operationType == "read" && it.operationScope.policyOperationScope == PolicyOperationScope.INTERNAL_AND_EXTERNAL }
...
//        val policy = doc.policy("test.TradeDeskPolicy")
//        val ruleSet = policy.rules.first { it.operationScope.operationType == "write" }
//        expect(ruleSet.operationScope.policyOperationScope).to.equal(PolicyOperationScope.INTERNAL_AND_EXTERNAL)
//    }
//
//    @Test
//    fun parsesFilterInstructionsCorrectly() {
//        val policy = doc.policy("test.TradeDeskPolicy")
//        val ruleSet = policy.rules.first { it.operationScope.operationType == "read" && it.operationScope.policyOperationScope == PolicyOperationScope.EXTERNAL }
