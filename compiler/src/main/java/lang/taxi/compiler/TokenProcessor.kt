...
//   private val calculatedFieldSetProcessor = CalculatedFieldSetProcessor(this)
   ...
   private fun compileViews() {
      this.tokens.unparsedViews.map { entry ->
         compileView(entry.key, entry.value.first, entry.value.second)
      }.invertEitherList()
         .flattenErrors()
   ...
   private fun compileView(
      ...
      val viewDocumentation = parseTypeDoc(viewCtx.typeDoc())
      val annotations = collateAnnotations(viewCtx.annotation())
      val modifiers = parseModifiers(viewCtx.typeModifier())
      val inherits = parseTypeInheritance(namespace, viewCtx.listOfInheritedTypes())
   ...
   private fun resolveViewBody(namespace: Namespace, bodyCtx: TaxiParser.FindBodyContext):
      ...
      return listTypeTypeOrError(joinTypes.first()).flatMap { bodyTypeType ->
         typeOrError(namespace, bodyTypeType).flatMap { bodyType ->
            if (joinTypes.size > 1) {
               val joinTypeTypeCtx = joinTypes[1]
               listTypeTypeOrError(joinTypeTypeCtx).flatMap { joinType ->
                  typeOrError(namespace, joinTypeTypeCtx).flatMap { joinType ->
   ...
   private fun parseViewBodyDefinition(namespace: Namespace, bodyCtx: TaxiParser.FindBodyContext): Either<List<CompilationError>, ViewBodyTypeDefinition> {
      ...
      val definitionErrors = typeBody.typeMemberDeclaration().map { typeMemberDeclarationContext ->
         ...
         resolveUserType(namespace, sourceTypeName, bodyCtx, SymbolKind.TYPE_OR_MODEL)
            .flatMap { sourceType ->
               val sourceFieldTypeName = classOrInterfaceTypeCtx.Identifier().last().text
               resolveUserType(namespace, sourceFieldTypeName, bodyCtx, SymbolKind.TYPE_OR_MODEL).flatMap { fieldSourceType ->
   ...
   private fun validateJoinBasedViewBodyDefinition(bodyCtx: TaxiParser.FindBodyContext, viewBodyDefinition: ViewBodyDefinition):
      ...
      val bodyHasAnIdField = bodyType.fields.filter { it.annotations.firstOrNull { annotation -> annotation.qualifiedName == JoinAnnotationName } != null }
      val joinHasAnIdField = joinType.fields.filter { it.annotations.firstOrNull { annotation -> annotation.qualifiedName == JoinAnnotationName } != null }
