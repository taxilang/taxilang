...
internal class QueryCompiler(private val tokenProcessor: TokenProcessor) {
    fun parseQueryBody(name: String, parameters: Map<String, QualifiedName>, ctx: TaxiParser.QueryBodyContext): Either<List<CompilationError>, TaxiQlQuery> {
   ...
   private fun parseQueryTypeList(queryTypeList: TaxiParser.QueryTypeListContext): Either<List<CompilationError>, List<DiscoveryType>> {
         ...
         tokenProcessor.parseType(namespace, queryType)
            .flatMap { type ->
               ...
               constraintsOrErrors.map { constraints ->
                  DiscoveryType(type.toQualifiedName(), constraints)
               }
            }
   ...
   private fun parseTypeToProject(queryProjection: TaxiParser.QueryProjectionContext?,
                                  typesToDiscover: List<DiscoveryType>): Either<List<CompilationError>, ProjectedType?> {
      ...
      if (anonymousProjectionType != null && concreteProjectionTypeType == null && typesToDiscover.size > 1) {
         return Either.left(listOf(CompilationError(queryProjection.start,
      ...
      return when {
            ...
            val isList = anonymousProjectionType.listType() != null
             this
               ...
               .parseAnonymousType(
                  ...
                  anonymousTypeResolutionContext = AnonymousTypeResolutionContext(typesToDiscover, concreteProjectionTypeType))
                .map { createdType ->
                   val compiledType =  if (isList) ArrayType(createdType, anonymousProjectionType.toCompilationUnit()) else createdType
                   ProjectedType.fomAnonymousTypeOnly(compiledType)
                }
         }

         concreteProjectionTypeType != null && anonymousProjectionType != null -> {
            val concreteProjectionType = this.tokenProcessor.parseType(queryProjection.findNamespace(), concreteProjectionTypeType)
            val typeBodyContext = anonymousProjectionType.typeBody()
            val isList = anonymousProjectionType.listType() != null
           ...
           return when {
               ...
               anonymousType is Either.Left -> anonymousType.a.left()
                  ...
                  val compiledType =  if (isList) ArrayType(createdAnonymousType, anonymousProjectionType.toCompilationUnit()) else createdAnonymousType
         ...
         concreteProjectionTypeType != null && anonymousProjectionType == null -> {
            this.tokenProcessor.parseType(queryProjection.findNamespace(), concreteProjectionTypeType).map { ProjectedType.fromConcreteTypeOnly(it) }

         }
         else -> Either.left(listOf(CompilationError(queryProjection.start,
            "projection type is a list but the type to discover is not, both should either be list or single entity.")))
...
data class AnonymousTypeResolutionContext(val typesToDiscover: List<DiscoveryType> = emptyList(), val concreteProjectionTypeContext: TaxiParser.TypeTypeContext? = null)
