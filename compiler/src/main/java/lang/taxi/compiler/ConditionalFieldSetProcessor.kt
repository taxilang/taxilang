...
class ConditionalFieldSetProcessor internal constructor(private val compiler: FieldCompiler, private val expressionCompiler: ExpressionCompiler) {
...
//    ) by when( xpath("/foo/bar") : String) { ...
   ...
   private fun compileWhenCondition(whenBlock: TaxiParser.ConditionalTypeWhenDeclarationContext, namespace: Namespace, whenSelectorType: Type): Either<List<CompilationError>, WhenFieldSetCondition> {
      return compileSelectorExpression(whenBlock.expressionGroup(), namespace, whenSelectorType).flatMap { selectorExpression ->
         val cases = compileWhenCases(whenBlock.conditionalTypeWhenCaseDeclaration(), selectorExpression.returnType, whenSelectorType)
   ...
   private fun compileWhenCase(whenCase: TaxiParser.ConditionalTypeWhenCaseDeclarationContext,
      ...
      return compileMatchExpression(whenCase.caseDeclarationMatchExpression())
         .flatMap { matchExpression ->
            typeChecker.ifAssignable(matchExpression.returnType, whenClauseSelectorType, whenCase) { matchExpression }.wrapErrorsInList()
         }
         .flatMap { matchExpression ->
               ...
               whenCase.scalarAccessorExpression() != null -> {
                  compileScalarFieldAssignment(whenCase.scalarAccessorExpression(), assignmentTargetType)
                     .flatMap { assignmentExpression ->
                        typeChecker.ifAssignable(assignmentExpression.returnType, assignmentTargetType, whenCase.scalarAccessorExpression()) {
...
//               }
   ...
   private fun compileScalarFieldAssignment(scalarAssigningDeclaration: TaxiParser.ScalarAccessorExpressionContext, whenClauseSelectorType: Type): Either<List<CompilationError>, AssignmentExpression> {
...
//         }
   ...
   private fun compileCaseScalarAssignment(caseScalarAssigningDeclaration: TaxiParser.CaseScalarAssigningDeclarationContext): Either<List<CompilationError>, Expression> {
      return when {
         caseScalarAssigningDeclaration.expressionGroup() != null -> expressionCompiler.compile(caseScalarAssigningDeclaration.expressionGroup())
...
//   }
   ...
   private fun compileMatchExpression(caseDeclarationMatchExpression: TaxiParser.CaseDeclarationMatchExpressionContext): Either<List<CompilationError>, Expression> {
      return when {
         caseDeclarationMatchExpression.expressionGroup() != null ->  expressionCompiler.compile(caseDeclarationMatchExpression.expressionGroup())
...
//   }
   ...
   private fun compileSelectorExpression(selectorExpression: TaxiParser.ExpressionGroupContext?, namespace: Namespace, targetType: Type): Either<List<CompilationError>, Expression> {
      return when {
