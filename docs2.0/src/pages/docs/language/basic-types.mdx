...
Array values can be read in two different ways:
...
model Person {
  friends: Person[]
  bestFriend : this.friends[0] // access using field name
  anotherWayOfSayingBestFriend: Person[][0] // access using type reference
}

// or, in a query:
given { index: Int = 3 } // can come from any expression
find { Person } as {
   friends: this.friends[index]
}
```

**Using a get function**
Taxi's stdlib has a number of functions for filtering and reading arrays, such as:
 * [`getAtIndex()`](/docs/language/stdlib#get-at-index)
 * [`first()`](/docs/language/stdlib#first)
 * [`last()`](/docs/language/stdlib#last)
...
<Callout type="warning" title="Using Collections">
  <ul>
    <li>Use arrays for ordered collections of the same type</li>
    <li>Only use maps when you need dynamic key-value relationships</li>
    <li>Consider creating a proper model instead of a map if the structure is known</li>
  </ul>
</Callout>
...
### Merging properties
For both Union Types and Intersection Types inherit all the properties of their
underlying types:
...
2. If both models have fields with the same name, but different types, the declaring model's name is prepended:
...
3. If both models have fields with the same name, and both models also have the same name, then the declaring models fully qualified name is prepended:
...
<Callout type={'note'}>
  Unlike in other languages, fields names are a lot less important in Taxi. Most mapping,
  orchestration, and transformation functions use Types, rather than field names.
</Callout>
...
A stream of an intersection type is generally stateful, so requires
some form of State Store provided by the TaxiQL engine. Read more about this
in State Stores.

### Limitations
Currently, Union and Intersection types are only supported as 
discovery types in a stream - as shown above.
