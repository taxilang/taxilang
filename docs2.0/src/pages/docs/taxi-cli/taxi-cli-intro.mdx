...
Read more details about the taxi.conf file [here](/language-reference/taxi-projects/#the-taxiconf-file)
...
You can leverage our existing plugins to generate code, or author your own.  Read more about plugins [here](/taxi-cli/plugins-intro/).
...
This command invokes the taxi compiler to validate your taxi code, and invokes any language generators configured.  (See [Plugins](/taxi-cli/plugins-intro).
...
A `docker-compose.yml` file is downloadded from [start.orbitalhq.com](https://start.orbitalhq.com), then runs `docker compose up`.
