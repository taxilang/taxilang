...
<Callout type="note" title="Service filtering vs TaxiQL expressions" content={`
...
<Callout type="warning" title="Common pitfall">
  Providing data in `given` helps with query execution but doesn't limit which operations can be called. To filter results or specify exact data requirements, use constraints in your `find` statement.
</Callout>
