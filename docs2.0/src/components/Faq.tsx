        ...
        <div className="lg:col-span-5">
          ...
          <p className="mt-4 text-lg leading-7 text-gray-400">
            ...
            <a href="https://join.slack.com/t/orbitalapi/shared_invite/zt-697laanr-DHGXXak5slqsY9DqwrkzHg"
               target="_blank"
               className="font-semibold text-sky-400 hover:underline underline-offset-4">
              Slack.
            </a>
          </p>
        </div>
            ...
            {faqs.map((faq, index) => (
              <Disclosure key={index} as="div" className="pt-6">
                {({open}) => (
                  <>
                    <dt>
                      <Disclosure.Button className="flex w-full text-white/80 transition-colors hover:text-white items-start justify-between text-left">
                        <span className={cn('text-base font-semibold leading-7', {'text-white': open})}>{faq.question}</span>
