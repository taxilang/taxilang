...
export default function QueryPlanOperationNode({ data }: Node<QueryPlanOperationNodeData>) {
  ...
  return (
    <div className="query-plan-operation-node">
      <Handle type="target" position={Position.Left} id={`${node.id}-left`} />
      <div className="query-plan-operation-node-header">
        <span className="query-plan-operation-node-title">{node.title}</span>
        <NodeBadge label={badgeLabel} kind={node.kind} iconId={node.icon} />
      </div>
      {node.members && node.members.length > 0 && (
        <div className="query-plan-operation-node-body">
          {node.members.map((member) => (
            <div key={member.handleId} className="query-plan-operation-node-member">
              <Handle
                type="target"
      ...
      <style jsx>{`
        ...
        .query-plan-operation-node-header {
          padding: 8px 12px;
        ...
        .query-plan-operation-node-title {
          font-weight: 600;
          font-size: 14px;
          color: ${theme.textColor};
        }

        .query-plan-operation-node-body {
          padding: 4px 0;
        }

        .query-plan-operation-node-member {
          padding: 4px 12px;
          display: flex;
          justify-content: space-between;
          align-items: center;
        ...
        .query-plan-operation-node-member:not(:last-child) {
