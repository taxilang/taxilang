...
import {ArrayType, ObjectType, Type, TypeAlias, TypeKind, UserType} from "./types";
...
export interface TypeMapper {
    ...
    getTypeOrError(node: ts.Node): Type;
}
...
export class DefaultTypeGenerator implements TypeMapper {
    ...
    getTypeOrError(node: ts.Node): Type {
        ...
        return this.getOrBuildTypeFromNode(node);
    }
    ...
    private lookupType(type: ts.TypeNode): Type {
        if (Primitives.forNodeKind(type.kind)) {
            ...
            let typeName = TypeHelper.getNameFromIdentifier(type.typeName);
            return this.getOrBuildType(typeName)
        } else if (ts.isArrayTypeNode(type)) {
            let memberType = this.lookupType(type.elementType);
    ...
    private getOrBuildTypeFromNode(node: ts.Node): Type {
        if (ts.isTypeReferenceNode(node)) {
            return this.getOrBuildType(TypeHelper.getNameFromIdentifier(node.typeName))
        } else if (ts.isArrayTypeNode(node)) {
            let arrayMemberType = this.getOrBuildTypeFromNode(node.elementType);
            return new ArrayType(arrayMemberType);
        } else {
            throw TODO("Not sure how to find type from node with kind = " + node.kind);
        }
    }
